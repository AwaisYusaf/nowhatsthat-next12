...
function Tag({ children }: any) {
  return (
    <Link
      ...
      className="bg-green-50 hover:bg-green-200 transition duration-300 text-xs mx-2 px-3 py-1 rounded-full font-medium"
    >
      {children}
    </Link>
  );
...
function LatestPost({ post }: { post: PostType }) {
  ...
  return (
    <section className="mb-4 md:mb-0 flex flex-col items-center lg:items-start">
      ...
      <div className="text-medium md:text-2xl text-lg mt-5 md:w-3/4 w-11/12 text-gray-500 leading-relaxed h-[320px] overflow-hidden">
